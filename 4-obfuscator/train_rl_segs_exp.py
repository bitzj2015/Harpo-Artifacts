...
parser.add_argument('--model_path', help="/SSD/baseline/bids", type=str, default="/SSD/baseline/oracle")
...
def train_url_generator(
    ...
    for ep in range(MAX_EP):
        ...
        print(len(attacker.rewards), reward)
        ...
        if np.mean(r_list[-10:]) > max_r:
            attacker.save_param("stealth")
            print("save")
            max_r = np.mean(r_list[-10:])
        csvFile = open("../DATA/rl_result/attacker_train_segs_{}_{}{}.csv".format(args.alpha, args.metric, TAG), 'a', newline='')
...
def test_url_generator( 
    ...
    test_persona = []
    test_persona_base = []
    stat = [{} for i in range(STAT)]
    for i in range(STAT):
        for j in range(193):
            stat[i][str(j)] = 0
    # with open("../DATA/rl_result/bias_weight_segs_{}_{}_{}_{}{}.json".format(args.keyword, args.iid, args.alpha, args.metric, TAG), "r") as file:
    #     random_weights_norm_ = np.array(json.load(file)["weight"])
    #     print(len(random_weights_norm_))
    ...
    for ep in range(MAX_EP):
        ...
        for l in range(len(attacker.env.reward_vec)):
            for k in range(len(attacker.env.reward_vec[l])):
                if attacker.env.reward_vec_base[l][k] == 0 and attacker.env.reward_vec[l][k] == 1:
            ...
            M2.append(seg_reward[l]["fake_seg"]/(seg_reward[l]["obfu_seg"]+1e-10))
    ...
    csvFile.close()
    with open("../DATA/rl_result/adap_{}_{}_{}_{}.json".format(args.keyword, args.iid, args.alpha, args.metric), "w") as file:
        json.dump(stat, file)
    stat_mat = np.zeros((STAT,STAT))
    for i in range(STAT):
        for j in range(STAT):
            stat_mat[i,j] = np.linalg.norm(np.array(list(stat[i].values())) - np.array(list(stat[j].values())))
    svm = sns.heatmap(stat_mat)
    sio.savemat('../DATA/rl_result/adap_{}_{}_{}_{}.mat'.format(args.keyword, args.iid, args.alpha, args.metric), {'mat':stat_mat})


    figure = svm.get_figure()    
    figure.savefig('adap_{}_{}.png'.format(args.keyword, args.iid), dpi=400)
