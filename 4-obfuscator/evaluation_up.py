...
for index in range(len(result_filename)):
    ...
    for key in result_dataset.keys():
        ...
        try:
            dataset[result_filename[index]][key_id]["bids"] = dict(zip(bids_list,[0 for _ in range(len(bids_list))]))
            dataset[result_filename[index]][key_id]["bids_cpm"] = dict(zip(bids_list,[0 for _ in range(len(bids_list))]))
...
for index in range(len(result_filename)-1):
    ...
    print(result_filename[index+1], np.mean(M1_list), np.mean(M2_list), np.mean(M3_list), np.mean(M4_list))
