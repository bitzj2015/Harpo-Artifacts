...
class doc2vec_model(object):
    def __init__(self, model):
        ...
        data = pickle.load(open("../DATA/crawl_result/AdNauseam_features.pkl", "rb"))
        random.seed(0)
        rd_url = list(data.keys())
        random.shuffle(rd_url)
        count = 0
        for url in rd_url:
            if len(data[url]) != 300:
                continue
            else:
                self.docvecs[url] = np.array(data[url])
                count += 1
                self.adnauseam_url["{}_{}".format("ads",count)] = {url:1}
                
            if count == 2000:
                break

        data = pickle.load(open("../DATA/crawl_result/TrackThis_features.pkl", "rb"))
        count = 0
        for url in data.keys():
            if len(data[url]) != 300:
                ...
                self.docvecs[url] = np.array(data[url])
                count += 1
                self.trackthis_url["{}_{}".format("tt",count)] = {url:1}

        data = pickle.load(open("../DATA/crawl_result/aol_features.pkl", "rb"))
        for url in data.keys():
            self.docvecs[url] = np.array(data[url])
